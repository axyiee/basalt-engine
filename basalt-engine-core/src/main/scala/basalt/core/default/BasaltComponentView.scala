...
class BasaltComponentView[F[_]: Sync](
  ...
  override def remove[C <: Component: ComponentFilterTag](
    ...
    for
      ...
      newArchetype <- archetype.removeComponent(target, componentId)
      action       <- entities.setArchetypeId(target, newArchetype.id)
    yield action
  ...
  override def set[C <: Component: ComponentFilterTag](
    ...
    for
      ...
      newArchetype <- archetype.addComponent(target, componentId, content)
      action       <- entities.setArchetypeId(target, newArchetype.id)
    yield action
