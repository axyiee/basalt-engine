...
import basalt.core.datatype.ComponentSet
...
trait ComponentView[F[_]]:
  ...
  /** Finds an archetype matching a set of components.
    *
    * @tparam I
    *   the iterable type of the components.
    * @return
    *   an instance of an archtype matching the components, if found.
    */
  def getArchetype[I <: QueryingFilterIterable: QueryingFilterIterableTag](using
  ...
  /** Extracts components matching the archetype component set and lifecycle
    * context such as added, changed, or removed.
    *
    * @tparam I
    *   the iterable type of the components
    * @return
    *   a stream returning the identification and component set of matching
    *   entities.
    */
  def extractWithinContext[
      I <: QueryingFilterIterable: QueryingFilterIterableTag
  ](using QueryingFilterIterableTag[I]): Stream[F, (EntityId, I)]

  /** Creates a [[ComponentSet]] holding the identification of components
    * matching the given [[QueryingFilterIterable]] type.
    *
    * @tparam I
    *   the iterable type of the components.
    * @return
    *   either a [[ComponentSet]] holding the identification of components
    *   matching the or a [[IllegalArgumentException]] if the iterable type is
    *   not supported.
    */
  def makeComponentSet[I <: QueryingFilterIterable](using
      QueryingFilterIterableTag[I]
