...
import basalt.core.datatype.{ComponentId, EntityId}
...
/** An agnostic type for detecting changes in the state of the engine between
  * ticks.
  */
trait InternalEvent(val entityId: EntityId, val componentId: ComponentId)
...
case class ComponentAdded(entityId: EntityId, componentId: ComponentId)
    extends InternalEvent(entityId, componentId)
...
type InternalEvents[F[_]] = Ref[F, List[InternalEvent]]
